...
public class AboutObjects {
	...
	public void toStringConcatenates() {
        ...
        /*
		    ...
		    Есть еще одна особенность в использовании анонимных класов. Поскольку имени у анонимного класа нет,
		    мы не можем для переменной где будет хранится обьект анонимного класа - указать "точный" тип обьекта
		    (имени ведь нет) - и нам приходится указывать как тип переменной - тип класа от которого наследуется
		    "анонимный клас", а это значит что мы сможем посылать обьекту внутри переменной только команды определенные
		    в класе-родителе. То есть, нет смысла создавать свои собственные команды-сообщения (публичные методы) для
		    анонимных класов, поскольку вы все равно не сможете их использовать. Но конечно можно создавать
		    непубличные (private) команды и переменнные внтутри анонимного класса - для внутреннего использования.

		    Еще одно обьяснение анонимных классов на видео: https://goo.gl/f8Yvbi
