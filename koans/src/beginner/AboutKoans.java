package beginner;
/*
    обычно каждому джава файлу с кодом "приваивается" своя фамилия (как минимум что бы не путать код с одинаковыми именами)
    В джава фамилия это - пакет (package), папка в которой живет файл с джава кодом (который должен иметь расширение .java)
...
/*
    ...
    слово public означает что эта "сущность", в даном случае - класс, может быть использована из вне, то есть в другом коде.
 */
...
/* Подсказки *
   	...
   	public void findAboutKoansFile(){
		...
		// обычно код состоит из набора команда
		// идентификатором команды джаве служыт набор символов (имя команды) за которыми следуют скобки с параметром внутри
		// или его остсутвие
	...
	public void definitionOfKoanCompletion(){
		...
		if(!koanIsComplete){
			...
			fail("what if koanIsComplete was true?");
			// соответственно так как у нас в ящике лежыт false, а if видит его результат его логического отрицания то есть
			// true - значит будет исполнен код в фигурных дужках - а там у нас живет команда fail
